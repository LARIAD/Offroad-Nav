...
dlio::OdomNode::OdomNode(ros::NodeHandle node_handle) : nh(node_handle), tf_buffer_(ros::Duration(10.0)) {
  ...
  this->trajectory.set_capacity(this->trajectory_buffer_size_);
  this->comp_times.set_capacity(this->comp_times_buffer_size_);
  this->imu_rates.set_capacity(this->imu_rates_buffer_size_);
  this->lidar_rates.set_capacity(this->lidar_rates_buffer_size_);
  this->metrics.spaciousness.set_capacity(this->metrics.spaciousness_buffer_size_);
  this->metrics.density.set_capacity(this->metrics.density_buffer_size_);
  this->keyframes.set_capacity(this->keyframes_buffer_size_);
  this->keyframe_timestamps.set_capacity(this->keyframe_timestamps_buffer_size_);
  this->keyframe_normals.set_capacity(this->keyframe_normals_buffer_size_);
  this->keyframe_transformations.set_capacity(this->keyframe_transformations_buffer_size_);
...
void dlio::OdomNode::getParams() {
  ...
  ros::param::param<bool>("~dlio/frames/concatenate", this->concatenate, true);
  ...
  // pub selector
  ros::param::param<bool>("~dlio/publish/keyframe", this->pub_keyframe_, true);
  ros::param::param<bool>("~dlio/publish/tf", this->pub_tf_, true);
  ros::param::param<bool>("~dlio/publish/path", this->pub_path_, true);
  ros::param::param<bool>("~dlio/publish/size_print", this->size_print, true);
  ...
  ros::param::param<bool>("~dlio/odom/imu/change_for_up", this->change_for_up, false);
  ...
  // Circular Buffers size
  ros::param::param<int>("~dlio/odom/buffers_size/trajectory", this->trajectory_buffer_size_, 1000);
  ros::param::param<int>("~dlio/odom/buffers_size/comp_times", this->comp_times_buffer_size_, 1000);
  ros::param::param<int>("~dlio/odom/buffers_size/imu_rates", this->imu_rates_buffer_size_, 1000);
  ros::param::param<int>("~dlio/odom/buffers_size/lidar_rates", this->lidar_rates_buffer_size_, 1000);
  ros::param::param<int>("~dlio/odom/buffers_size/spaciousness", this->metrics.spaciousness_buffer_size_, 1000);
  ros::param::param<int>("~dlio/odom/buffers_size/density", this->metrics.density_buffer_size_, 1000);
  ros::param::param<int>("~dlio/odom/buffers_size/keyframes", this->keyframes_buffer_size_, 100);
  ros::param::param<int>("~dlio/odom/buffers_size/keyframe_timestamps", this->keyframe_timestamps_buffer_size_, 100);
  ros::param::param<int>("~dlio/odom/buffers_size/keyframe_normals", this->keyframe_normals_buffer_size_, 100);
  ros::param::param<int>("~dlio/odom/buffers_size/keyframe_transformations", this->keyframe_transformations_buffer_size_, 100);

}

void dlio::OdomNode::loadOdomFromTf(){
  // wait until the static TFs have appeared
  bool check=true;
  while (check){
    check=!tf_buffer_.canTransform(baselink_frame, lidar_frame,  ros::Time(0), ros::Duration(2.0));
  }
  auto lidar_tf = tf_buffer_.lookupTransform(  baselink_frame, lidar_frame, ros::Time(0));
  Eigen::Isometry3d T_bl_lidar = tf2::transformToEigen(lidar_tf.transform);
  t_odom = T_bl_lidar.translation().cast<float>();
  R_odom = T_bl_lidar.rotation().cast<float>();
  T_transodom = (T_bl_lidar.matrix()).cast<float>();
  Eigen::Matrix4f inv;
  inv=T_transodom.inverse();
  origin_offset=inv.block<3,1>(0,3);
  first=true;
...
void dlio::OdomNode::publishPose(const ros::TimerEvent& e) {
  ...
  Eigen::Matrix4f Trans,Trans_inv,tottrans;
  Trans.setIdentity();
  Trans.block<3,3>(0,0) = this->state.q.toRotationMatrix ();
  Trans.block<3,1>(0,3) = this->state.p;
  Trans_inv=T_transodom.inverse();
  tottrans=Trans*(T_transodom.inverse());
  Eigen::Vector3f pos(0.0,0.0,0.0);
  Eigen::Vector4f result_pos; 
  result_pos=tottrans*pos.homogeneous();
  Eigen::Quaternionf qtrue(tottrans.block<3,3>(0,0));
  if (first){
    origin_offset=-(this->state.q*origin_offset);
    first=false;
  }
  this->odom_ros.pose.pose.position.x = result_pos[0];
  this->odom_ros.pose.pose.position.y = result_pos[1];
  this->odom_ros.pose.pose.position.z = result_pos[2]; 

  this->odom_ros.pose.pose.orientation.w = qtrue.w();
  this->odom_ros.pose.pose.orientation.x = qtrue.x();
  this->odom_ros.pose.pose.orientation.y = qtrue.y();
  this->odom_ros.pose.pose.orientation.z = qtrue.z(); 
  Eigen::Vector3f vlin,vang;
  Eigen::Vector3f lidar2base=R_odom.inverse() * t_odom;
  vlin=this->state.v.lin.b+this->state.v.ang.b.cross(t_odom);
  vlin=R_odom*vlin;
  this->odom_ros.twist.twist.linear.x = vlin[0];
  this->odom_ros.twist.twist.linear.y = vlin[1];
  this->odom_ros.twist.twist.linear.z = vlin[2];
  vang=R_odom*this->state.v.ang.b;
  this->odom_ros.twist.twist.angular.x = vang[0];
  this->odom_ros.twist.twist.angular.y = vang[1];
  this->odom_ros.twist.twist.angular.z = vang[2];
...
void dlio::OdomNode::callbackPointCloud(const sensor_msgs::PointCloud2ConstPtr& pc) {
  ...
  if (!this->verbose && this->size_print) {
    this->debug_thread = std::thread(&dlio::OdomNode::print_vector_size, this );
    this->debug_thread.detach();
  }
...
sensor_msgs::Imu::Ptr dlio::OdomNode::transformImu(const sensor_msgs::Imu::ConstPtr& imu_raw) {
  ...
  Eigen::Vector3f ang_vel;
  if (! this->change_for_up) {
    ang_vel=Eigen::Vector3f(imu_raw->angular_velocity.x,
                          imu_raw->angular_velocity.y,
                          imu_raw->angular_velocity.z);
    
  } else {
    ang_vel=Eigen::Vector3f(imu_raw->angular_velocity.y,
  ...
  Eigen::Vector3f lin_accel;
  if (!this->change_for_up) {
    lin_accel=Eigen::Vector3f(imu_raw->linear_acceleration.x,
                          imu_raw->linear_acceleration.y,
                          imu_raw->linear_acceleration.z);
    
  } else {
    lin_accel=Eigen::Vector3f(imu_raw->linear_acceleration.y,
...
void dlio::OdomNode::buildSubmap(State vehicle_state) {
  ...
  bool check_full=this->keyframes.full();
  lock.unlock();
  ...
  // prepare previous vector if kfames circular buffer is full
  int size_of_prev=this->submap_kf_idx_prev.size();
  int size_of_curr=this->submap_kf_idx_curr.size();

  if (size_of_prev>0 and check_full and size_of_curr>0){
    // check if first ellement in previous vector was the first kframe
    if (this->submap_kf_idx_prev[0]==0 and size_of_prev>1){
      // compute index shift due to circular update 
      int diff=this->submap_kf_idx_curr[0]-this->submap_kf_idx_prev[1];
      // apply shift
      for (int i=1;i<size_of_prev; i++){
        this->submap_kf_idx_prev[i-1]=this->submap_kf_idx_prev[i]+diff;
      }
      // remove last element because first disappeared to kframe buffer
      this->submap_kf_idx_prev.pop_back();
...
void dlio::OdomNode::print_vector_size() {
  printf("\033[2J\033[1;1H");

  std::cout << std::endl
  << "+-------------------------------------------------------------------+" << std::endl;
  std::cout << "+ size of:" << std::endl;
  std::cout << "+ trajectory: " << this->trajectory.size() << std::endl;
  ...
  std::cout << "+ keyframe_convex: " << this->keyframe_convex.size() << std::endl;
  std::cout << "+ keyframe_concave: " << this->keyframe_concave.size() << std::endl;
  std::cout << "+ submap_kf_idx_curr: " << this->submap_kf_idx_curr.size() << std::endl;
  std::cout << "+ submap_kf_idx_prev: " << this->submap_kf_idx_prev.size() << std::endl;
  std::cout << "+ comp_times: " << this->comp_times.size() << std::endl;
  std::cout << "+ imu_rates: " << this->imu_rates.size() << std::endl;
  std::cout << "+ lidar_rates: " << this->lidar_rates.size() << std::endl;
  std::cout << "+ cpu_percents: " << this->cpu_percents.size() << std::endl;
  std::cout << "+ spaciousness: " << this->metrics.spaciousness.size() << std::endl;
  std::cout << "+ density: " << this->metrics.density.size() << std::endl;
  std::cout << "+-------------------------------------------------------------------+" << std::endl;
}
