...
private:
  ...
  boost::circular_buffer<std::pair<std::pair<Eigen::Vector3f, Eigen::Quaternionf>,
                        pcl::PointCloud<PointType>::ConstPtr>> keyframes;
  boost::circular_buffer<ros::Time> keyframe_timestamps;
  boost::circular_buffer<std::shared_ptr<const nano_gicp::CovarianceList>> keyframe_normals;
  boost::circular_buffer<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> keyframe_transformations;
  std::mutex keyframes_mutex;
  int keyframes_buffer_size_;
  int keyframe_timestamps_buffer_size_;
  int keyframe_normals_buffer_size_;
  int keyframe_transformations_buffer_size_;
  ...
  int comp_times_buffer_size_;
  int imu_rates_buffer_size_;
  int lidar_rates_buffer_size_;
  ...
  // Transformations
  Eigen::Matrix4f T, T_prior, T_corr,T_transodom;
  Eigen::Quaternionf q_final;

  Eigen::Vector3f origin,t_odom,origin_offset;
  ...
  struct Metrics {
    ...
    boost::circular_buffer<float> spaciousness;
    boost::circular_buffer<float> density;
    int spaciousness_buffer_size_;
    int density_buffer_size_;
  }; Metrics metrics;
