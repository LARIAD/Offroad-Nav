...
void SbgDevice::publishInternalStaticTF()
{
  // base_link -> imu_link
  if (config_store_.getPublishFrameTF()) {
    geometry_msgs::TransformStamped t;
    t.header.stamp = ros::Time::now();
    t.header.frame_id = config_store_.getOdomBaseFrameId();
    t.child_frame_id = config_store_.getFrameId();
    // the signs are chosen given that the lever arms are in the sbg frame (as configured with axisDirectionX/axisDirectionY)
    // FROM the IMU TO the robot's center of rotation
    t.transform.translation.x = - config_store_.getSensorLevelArms()(0);
    t.transform.translation.y = config_store_.getSensorLevelArms()(1);
    t.transform.translation.z = config_store_.getSensorLevelArms()(2);
    // the orientation here are to compensate the signs that are applied on the raw data depending on use_enu
    t.transform.rotation.x = config_store_.getUseEnu() ? 0 : 1;
    t.transform.rotation.y = 0;
    t.transform.rotation.z = 0;
    t.transform.rotation.w = config_store_.getUseEnu() ? 1 : 0;
    static_tf_broadcaster_.sendTransform(t);
  }

  // imu_link -> sbg_ant1_apc
  if (config_store_.getPublishPrimaryAntennaTF())
  {
    const tf2::Vector3 lAP{
      config_store_.getGnssInstallation().leverArmPrimary[0],
      config_store_.getGnssInstallation().leverArmPrimary[1],
      config_store_.getGnssInstallation().leverArmPrimary[2]
    };
    const tf2::Vector3 lAS{
      config_store_.getGnssInstallation().leverArmSecondary[0],
      config_store_.getGnssInstallation().leverArmSecondary[1],
      config_store_.getGnssInstallation().leverArmSecondary[2]
    };
    const tf2::Vector3 lAPS = lAS - lAP;
    // === Compute offset between base_link x vector to vec(primary apc, secondary apc)
    // this rotation is for the GNSS heading to be properly defined
    const double yaw = atan2(config_store_.getUseEnu() ? -lAPS.y() : lAPS.y(), lAPS.x());
    const double pitch = -atan2(config_store_.getUseEnu() ? -lAPS.z() : lAPS.z(),
                                sqrt(pow(lAPS.x(), 2) + pow(lAP.y(), 2)));
    tf2::Quaternion q;
    q.setRPY(0, pitch, yaw);
    config_store_.setRobotHeadingOffset(q);

    // === Publish TF
    geometry_msgs::TransformStamped t;
    t.header.stamp = ros::Time::now();
    t.header.frame_id = config_store_.getFrameId();
    t.child_frame_id = config_store_.getPrimaryAntennaFrameId();
    // the signs are chosen given that the lever arms are in the sbg frame (as configured with axisDirectionX/axisDirectionY)
    // FROM the IMU TO the primary antenna APC
    // Also the orientation of the imu_link changes depending on the use_enu parameter, which is not the case for the
    // primary lever arms (theory are ALWAYS the same no matter use_enu parameter).
    t.transform.translation.x = lAP.x();
    t.transform.translation.y = config_store_.getUseEnu() ? -lAP.y() : lAP.y();
    t.transform.translation.z = config_store_.getUseEnu() ? -lAP.z() : lAP.z();
...
void SbgDevice::readDeviceInfo(void)
{
  ...
  {
    ROS_ERROR("Unable to get the device Info : %s, rebooting device...", sbgErrorCodeToString(error_code));
    ros::shutdown();
  }
}
