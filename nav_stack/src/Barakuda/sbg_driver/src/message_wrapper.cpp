...
first_valid_utc_(false), tf_listener_(tf_buff_)
{
}
...
void MessageWrapper::setPrimaryAntennaFrameId(const std::string &frame_id)
{
  primary_antenna_frame_id_ = frame_id;
}
...
void MessageWrapper::setOdomMapFrameId(const std::string &ref_frame_id)
{
  odom_map_frame_id_ = ref_frame_id;
}

void MessageWrapper::setRobotHeadingOffset(const tf2::Quaternion &q)
{
  robot_to_gnss_offset = q;
}
...
const sbg_driver::SbgEkfEuler MessageWrapper::createSbgEkfEulerMessage(const SbgLogEkfEulerData& ref_log_ekf_euler) const
{
  ...
  ekf_euler_message.header.frame_id = odom_base_frame_id_;
...
const sbg_driver::SbgEkfNav MessageWrapper::createSbgEkfNavMessage(const SbgLogEkfNavData& ref_log_ekf_nav) const
{
  ...
  ekf_nav_message.header.frame_id = odom_base_frame_id_;
...
const sbg_driver::SbgEkfQuat MessageWrapper::createSbgEkfQuatMessage(const SbgLogEkfQuatData& ref_log_ekf_quat) const
{
  ...
  ekf_quat_message.header.frame_id = odom_base_frame_id_;
...
const sbg_driver::SbgGpsHdt MessageWrapper::createSbgGpsHdtMessage(const SbgLogGpsHdt& ref_log_gps_hdt) const
{
  ...
  gps_hdt_message.header.frame_id  = primary_antenna_frame_id_;
...
const sbg_driver::SbgGpsPos MessageWrapper::createSbgGpsPosMessage(const SbgLogGpsPos& ref_log_gps_pos) const
{
  ...
  gps_pos_message.header.frame_id    = primary_antenna_frame_id_;
...
const sbg_driver::SbgGpsVel MessageWrapper::createSbgGpsVelMessage(const SbgLogGpsVel& ref_log_gps_vel) const
{
  ...
  gps_vel_message.header.frame_id    = primary_antenna_frame_id_;
...
{
  // **BE AWARE** that this can be called with SbgImuData filled using sbg_driver::SbgImuShort (some field might be empty).
  // See the next definitions of MessageWrapper::createRosOdoMessage.
  ...
  if (!utm_.isInit())
  {
    ...
    auto first_valid_easting_ = first_valid_easting_northing[0];
    auto first_valid_northing_ = first_valid_easting_northing[1];
    auto first_valid_altitude_ = ref_ekf_nav_msg.altitude;
    ...
    ROS_INFO("Initialized from lat:%f long:%f UTM zone %d%c: easting:%fm (%dkm) northing:%fm (%dkm)"
             ...
             , first_valid_easting_, static_cast<int>(first_valid_easting_) / 1000
             , first_valid_northing_, static_cast<int>(first_valid_northing_) / 1000
    );

    // when driver is in charge of publishing utm -> map -> odom
    if (odom_publish_tf_) {
      // Prepare pose to compute tf.
      geometry_msgs::Pose utm_to_map;
      utm_to_map.position.x = first_valid_easting_;
      utm_to_map.position.y = first_valid_northing_;
      utm_to_map.position.z = first_valid_altitude_;
      utm_to_map.orientation.w = 1;

      // Publish utm -> map (initial position)
      fillTransform(odom_init_frame_id_, odom_map_frame_id_, utm_to_map, transform);
      static_tf_broadcaster_.sendTransform(transform);

      // Save the utm -> map tf for latter use
      tf2::Transform tf;
      tf2::fromMsg(transform.transform, tf);
      tf = tf.inverse();
      tf_map_to_utm.header = transform.header;
      tf_map_to_utm.child_frame_id = transform.child_frame_id;
      tf_map_to_utm.transform = tf2::toMsg(tf);

      // Publish map -> odom (initial orientation)
      geometry_msgs::Pose map_to_odom;
      tf2::convert(ref_orientation, map_to_odom.orientation);
      fillTransform(odom_map_frame_id_, odom_map_frame_id_, map_to_odom, transform);
      static_tf_broadcaster_.sendTransform(transform);
    }
  }

  // when driver is not in charge of publish the utm -> map -> odom tfs, subscribe to the tf
  if (!odom_publish_tf_)
  {
    try
    {
      tf_map_to_utm = tf_buff_.lookupTransform(
        odom_map_frame_id_,
        odom_init_frame_id_,
        ros::Time::now(),
        ros::Duration(.1));
    ...
    {
      ROS_WARN("Error while retrieving tf %s -> %s: %s",
               odom_map_frame_id_.c_str(), odom_init_frame_id_.c_str(), e.what());
    }
  }
  ...
  odo_ros_msg.header.frame_id = odom_map_frame_id_;

  // build map to base_link transform
  const auto easting_northing = utm_.computeEastingNorthing(ref_ekf_nav_msg.latitude, ref_ekf_nav_msg.longitude);
  geometry_msgs::Pose utm_to_base_link;
  tf2::convert(ref_orientation, utm_to_base_link.orientation);
  utm_to_base_link.position.x = easting_northing[0];
  utm_to_base_link.position.y = easting_northing[1];
  utm_to_base_link.position.z = ref_ekf_nav_msg.altitude;

  // transforms it to odom to base_link tf
  tf2::doTransform(utm_to_base_link, odo_ros_msg.pose.pose, tf_map_to_utm);
  ...
  odo_ros_msg.child_frame_id            = odom_base_frame_id_;
  ...
  if (odom_publish_tf_)
  {
    ...
    // TODO this might be a problem for move base
    fillTransform(odom_map_frame_id_, odom_base_frame_id_, odo_ros_msg.pose.pose, transform);
...
{
  /*
  createRosOdoMessage for SbgImuShort (https://github.com/SBG-Systems/sbg_ros_driver/blob/master/src/message_wrapper.cpp#L1007-L1094)
  is identical to the one for SbgImuData (https://github.com/SBG-Systems/sbg_ros_driver/blob/master/src/message_wrapper.cpp#L901-L988)
  if SbgImuData.gyro = SbgImuShort.delta_angle and SbgImuData.time_stamp = SbgImuShort.time_stamp.
  Reduce code duplication it was used to replace createRosOdoMessage for SbgImuShort by this function.
   */
  sbg_driver::SbgImuData ref_sbg_imu_full_msg;
  ref_sbg_imu_full_msg.time_stamp = ref_sbg_imu_msg.time_stamp;
  ref_sbg_imu_full_msg.gyro = ref_sbg_imu_msg.delta_angle;

  return createRosOdoMessage(ref_sbg_imu_full_msg, ref_ekf_nav_msg, ref_orientation, ref_ekf_euler_msg);
}
...
const sensor_msgs::NavSatFix MessageWrapper::createRosNavSatFixMessage(const sbg_driver::SbgGpsPos& ref_sbg_gps_msg) const
{
  ...
  nav_sat_fix_message.header.frame_id    = primary_antenna_frame_id_;
  ...
  nav_sat_fix_message.status.service = 0;
  ...
  if (ref_sbg_gps_msg.status.gps_l1_used || ref_sbg_gps_msg.status.gps_l2_used)
  {
...
const sensor_msgs::Imu MessageWrapper::createRosGNSSImuMessage(const sbg_driver::SbgGpsHdt& sbg_gps_hdt_message_) const
{
  sensor_msgs::Imu imu_message;
  imu_message.header = createRosHeader(sbg_gps_hdt_message_.time_stamp);
  imu_message.header.frame_id = odom_base_frame_id_;
  // mark angular vel and linear accel as not measured
  imu_message.angular_velocity_covariance[0] = -1;
  imu_message.linear_acceleration_covariance[0] = -1;

  // compute robot orientation
  tf2::Quaternion world_to_gnss;
  world_to_gnss.setRPY(0,
                       sbg_gps_hdt_message_.pitch * M_PI / 180,
                       sbg_gps_hdt_message_.true_heading * M_PI / 180);
  tf2::Quaternion world_to_robot = world_to_gnss * robot_to_gnss_offset.inverse();
  tf2::convert(world_to_robot, imu_message.orientation);

  // fill covar
  imu_message.orientation_covariance[0] = 0.0;
  imu_message.orientation_covariance[1] = 0.0;
  imu_message.orientation_covariance[2] = 0.0;
  imu_message.orientation_covariance[3] = 0.0;
  imu_message.orientation_covariance[4] = pow(sbg_gps_hdt_message_.pitch_acc * M_PI / 180.0, 2);
  imu_message.orientation_covariance[5] = 0.0;
  imu_message.orientation_covariance[6] = 0.0;
  imu_message.orientation_covariance[7] = 0.0;
  imu_message.orientation_covariance[8] = pow(sbg_gps_hdt_message_.true_heading_acc * M_PI / 180.0, 2);
  return imu_message;
}
