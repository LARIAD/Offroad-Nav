...
void MessagePublisher::defineRosStandardPublishers(ros::NodeHandle& ref_ros_node_handle, bool odom_enable, bool enu_enable)
{
  ...
  {
    imu_pub_ = ref_ros_node_handle.advertise<sensor_msgs::Imu>(imu_node_name + "/data", max_messages_);
  }
  else
  {
  ...
  {
    temp_pub_ = ref_ros_node_handle.advertise<sensor_msgs::Temperature>(imu_node_name + "/temp", max_messages_);
  }
  else
  {
  ...
  if (sbg_mag_pub_)
  {
    mag_pub_ = ref_ros_node_handle.advertise<sensor_msgs::MagneticField>(imu_node_name + "/mag", max_messages_);
  }
  else
  {
  ...
  {
    fluid_pub_ = ref_ros_node_handle.advertise<sensor_msgs::FluidPressure>(imu_node_name + "/pres", max_messages_);
  }
  else
  {
  ...
  if (sbg_ekf_nav_pub_)
  {
    pos_ecef_pub_ = ref_ros_node_handle.advertise<geometry_msgs::PointStamped>(imu_node_name + "/pos_ecef", max_messages_);
  }
  else
  {
  ...
  {
    utc_reference_pub_ = ref_ros_node_handle.advertise<sensor_msgs::TimeReference>(imu_node_name + "/utc_ref", max_messages_);
  }
  else
  {
  ...
  if (sbg_gps_pos_pub_)
  {
    nav_sat_fix_pub_ = ref_ros_node_handle.advertise<sensor_msgs::NavSatFix>(imu_node_name + "/nav_sat_fix", max_messages_);
  }
  else
  {
  ...
  if (sbg_gps_hdt_pub_)
  {
    gnss_imu_pub_ = ref_ros_node_handle.advertise<sensor_msgs::Imu>(imu_node_name + "/hdt", max_messages_);
  }
  else
  {
    ROS_WARN("SBG_DRIVER - [Publisher] SBG GPS hdt data output are not configured, the GNSS IMU publisher can not be defined.");
  }
...
void MessagePublisher::initPublishers(ros::NodeHandle& ref_ros_node_handle, const ConfigStore &ref_config_store)
{
  ...
  message_wrapper_.setPrimaryAntennaFrameId(ref_config_store.getPrimaryAntennaFrameId());
  ...
  message_wrapper_.setOdomMapFrameId(ref_config_store.getOdomMapFrameId());
  message_wrapper_.setOdomInitFrameId(ref_config_store.getOdomInitFrameId());
  message_wrapper_.setRobotHeadingOffset(ref_config_store.getRobotHeadingOffset());
...
void MessagePublisher::publish(SbgEComClass sbg_msg_class, SbgEComMsgId sbg_msg_id, const SbgBinaryLogData &ref_sbg_log)
{
  ...
  if(sbg_msg_class == SBG_ECOM_CLASS_LOG_ECOM_0)
  {
    ...
    case SBG_ECOM_LOG_GPS2_HDT:
      if (sbg_gps_hdt_pub_)
      {
        auto sbg_gps_hdt_message_ = message_wrapper_.createSbgGpsHdtMessage(ref_sbg_log.gpsHdtData);
        sbg_gps_hdt_pub_.publish(sbg_gps_hdt_message_);
        if (gnss_imu_pub_)
        {
          gnss_imu_pub_.publish(message_wrapper_.createRosGNSSImuMessage(sbg_gps_hdt_message_));
        }
      }
      break;
