...
## How to properly configure this driver

1. Copy the [ellipse_D_default.yaml](config/example/ellipse_D_default.yaml) default to bringup of the robot
2. Ensure that the communication parameters are correct (see `uartConf`)
3. Ensure that `odometry/enable`, `odometry/confWithRos`, `use_enu` are `true`
4. If this driver is responsible for the `map` -> `odom` -> `base_link` (e.g. there is no robot localisation),
   `odometry/publishTf` should be `true`. Also, check the frame names `odomFrameId`, `baseFrameId` and `initFrameId`.
5. Measure the distances along the X, Y, Z axis of the SBG frame (the one drawn on the SBG itself) _from_ the SBG center
   of measurements _to_ the robot's center of rotation. /!\ This step was tested with `axisDirectionX=0` and
   `axisDirectionY=3`, other values might affect the signs of these distances /!\. Those values are the
   `imuAlignementLeverArm/leverArm[X,Y,Z]`. According to SBG, they should be measured with an accuracy of 1 cm.
6. Measure the distances along the X, Y, Z axis of the SBG frame (the one drawn on the SBG itself) _from_ the SBG center
   of measurements _to_ each antenna phase center. /!\ This step was tested with `axisDirectionX=0` and
   `axisDirectionY=3`, other values might affect the signs of these distances /!\. Those values are the
   `gnss/[primary,secondary]LeverArmX[X,Y,Z]`. If the accuracy of these measurements are under 5cm, the
   `gnss/primaryLeverPrecise` can be set to true and the `gnss/secondaryLeverMode` can be set to 4. 
7. Check the frame names `gnss/primary_antenna_frame_id` and `output/frame_id`. If no other system publishes those
   frames (e.g. URDF), the driver can publish them using `gnss/publish_primary_antenna_tf` and
   `output/publish_frame_tf`.
8. Do the magnetometer calibration (see below).
