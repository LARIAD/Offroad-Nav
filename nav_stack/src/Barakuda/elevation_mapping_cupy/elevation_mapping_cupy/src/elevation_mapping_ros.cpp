...
ElevationMappingNode::ElevationMappingNode(ros::NodeHandle& nh)
  ...
  nh.param<std::string>("layer_to_point_cloud",layer_to_pointcloud_,"elevation");
...
void ElevationMappingNode::inputPointCloud(const sensor_msgs::PointCloud2& cloud,
  ...
  tf::StampedTransform transformTf,transform_pcl2bl;
  std::string sensorFrameId = cloud.header.frame_id;
  auto timeStamp = cloud.header.stamp;
  Eigen::Affine3d transformationSensorToMap,transformationSensorToBase;
  try {
  ...
  //std::pair<std::string,Eigen::VectorXd> static_trans;
  if (auto static_trans=sensors_trans_.find(sensorFrameId);static_trans == sensors_trans_.end()){
    try {
      transformListener_.waitForTransform(sensorFrameId,baseFrameId_, timeStamp, ros::Duration(10.0));
      transformListener_.lookupTransform(sensorFrameId,baseFrameId_, timeStamp, transform_pcl2bl);
      poseTFToEigen(transform_pcl2bl,transformationSensorToBase);
      this->sensors_trans_[sensorFrameId]=transformationSensorToBase.translation();
    } catch (tf::TransformException& ex) {
      ROS_ERROR("%s", ex.what());
      return;
    }
  }
  Eigen::VectorXd trans_sensorbase(sensors_trans_[sensorFrameId]);
  ...
  Eigen::VectorXd translate_pp(transformationSensorToMap.translation().size()+trans_sensorbase.size());
  translate_pp << transformationSensorToMap.translation(),trans_sensorbase;
  
  map_.input(points, channels, transformationSensorToMap.rotation(), translate_pp, positionError,
             orientationError);
...
void ElevationMappingNode::publishAsPointCloud(const grid_map::GridMap& map) const {
  ...
  grid_map::GridMapRosConverter::toPointCloud(map, layer_to_pointcloud_ , msg);
