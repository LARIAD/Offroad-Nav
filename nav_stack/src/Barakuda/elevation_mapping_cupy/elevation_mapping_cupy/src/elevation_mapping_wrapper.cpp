...
void ElevationMappingWrapper::setParameters(ros::NodeHandle& nh) {
  ...
  // point cloud channel should add to pcl for exec fusion
  if (!nh.hasParam("channels_should_add_to_pcl_for_exec_fusion")) {
    ROS_WARN("No channels_should_add_to_pcl_for_exec_fusion parameter found. Using default values.");
  }
  else {
    XmlRpc::XmlRpcValue channels_should_add_to_pcl_for_exec_fusion;
    nh.getParam("channels_should_add_to_pcl_for_exec_fusion", channels_should_add_to_pcl_for_exec_fusion);

    py::dict channels_should_add_to_pcl_for_exec_fusion_dict;
    for (auto& channels_should_add : channels_should_add_to_pcl_for_exec_fusion) {
      const char* const name = channels_should_add.first.c_str();
      std::string should_add = static_cast<std::string>(channels_should_add.second);
      if (!channels_should_add_to_pcl_for_exec_fusion_dict.contains(name)) {
        channels_should_add_to_pcl_for_exec_fusion_dict[name] = should_add;
      }
    }
    ROS_INFO_STREAM("channels_should_add_to_pcl_for_exec_fusion_dict: " << channels_should_add_to_pcl_for_exec_fusion_dict);
    param_.attr("channels_should_add_to_pcl_for_exec_fusion") = channels_should_add_to_pcl_for_exec_fusion_dict;
  }
