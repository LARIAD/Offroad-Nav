...
class PluginBase(ABC):
    ...
    def get_layer_data(
        ...
        if name in layer_names:
            ...
            if (time.time() - self._print_time > 0.3):
...
class PluginManager(object):
        ...
        for param, extra_param in zip(plugin_params, extra_params):
            ...
            for name, obj in inspect.getmembers(m):
                if (
                    ...
                    extra_param["layer_name"]=param.layer_name
                    if param.apply_shift:
                        self.list_of_shiftable.append(param.layer_name)
    ...
    def load_plugin_settings(self, file_path: str):
        ...
        print("param extract from ",file_path)
            ...
            if v["enable"]:
                plugin_params.append(
                    PluginParams(
                        ...
                        apply_shift=False if not "apply_shift" in v else v["apply_shift"],
                    )
                )
                extra_params.append(v["extra_params"])
        self.init(plugin_params, extra_params)
        print("Loaded plugins are ", *self.plugin_names," with extra param: ",extra_params)
