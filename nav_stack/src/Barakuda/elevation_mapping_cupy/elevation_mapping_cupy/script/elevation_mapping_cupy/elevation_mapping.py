...
class ElevationMap:
    ...
    def shift_map_xy(self, delta_pixel):
        ...
        with self.map_lock:
            ...
            for name_plug in self.plugin_manager.list_of_shiftable:
                ids_plug=self.plugin_manager.layer_names.index(name_plug)
                #print("plgin layer that roll is ",self.plugin_manager.layer_names[ids_plug])
                self.plugin_manager.layers[ids_plug]=cp.roll(self.plugin_manager.layers[ids_plug],shift_value,axis=(0,1))
                if shift_value[0] > 0:
                    self.plugin_manager.layers[ids_plug][: shift_value[0], :] = 0.0
                elif shift_value[0] < 0:
                    self.plugin_manager.layers[ids_plug][shift_value[0] :, :] = 0.0
                if shift_value[1] > 0:
                    self.plugin_manager.layers[ids_plug][:, : shift_value[1]] = 0.0
                elif shift_value[1] < 0:
                    self.plugin_manager.layers[ids_plug][:, shift_value[1] :] = 0.0
    ...
    def update_map_with_kernel(self, points_all, channels, R, t, position_noise, orientation_noise):
        ...
        points = cp.copy(points_all[:, :3])
        new_t=cp.copy(t)
        t=cp.array([new_t[0],new_t[1],new_t[2]])
        ...
        with self.map_lock:
            ...
            #print("cooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooool") 
            self.semantic_map.update_layers_pointcloud(points_all, channels, R, cp.array([new_t[0],new_t[1],new_t[2],self.center[0],self.center[1],self.center[2],new_t[3],new_t[4],new_t[5]]), self.new_map)
