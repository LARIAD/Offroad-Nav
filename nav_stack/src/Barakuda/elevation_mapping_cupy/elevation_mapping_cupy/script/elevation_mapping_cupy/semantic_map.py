...
class SemanticMap:
    def __init__(self, param: Parameter):
        ...
        # print("uhyyuhuybyuvufuf   ",self.param.channels_should_add_to_pcl_for_exec_fusion)
        self.bonus_channels=self.param.channels_should_add_to_pcl_for_exec_fusion
    ...
    def get_fusion(
        ...
        fusion_list = []
        process_channels = []
        #print("chchchchchchchchchchchchch: ",channels)
        for channel in channels:
            if channel not in layer_specs:
                ...
                if matched_fusion is None:
                    ...
                    elif "again" in channel_fusions:
                        default_fusion = channel_fusions["again"]
                        print(
                            f"[WARNING] Layer {channel} not found in layer_specs. Using {default_fusion} algorithm as default."
                        )
                        layer_specs[channel] = default_fusion
    ...
    def get_indices_fusion(self, pcl_channels: List[str], fusion_alg: str, layer_specs: Dict[str, str]):
        ...
        """
        # this contains exactly the fusion alg type for each channel of the pcl and add channels expected by user
        new_pcl_channels=pcl_channels+list(self.bonus_channels)
        pcl_val_list = [layer_specs[x] for x in new_pcl_channels]
        ...
        layer_indices = cp.array([], dtype=cp.int32)
        for it, (key, val) in enumerate(layer_specs.items()):
            if key in new_pcl_channels and val == fusion_alg:
                # get first index of layers that wanted by user
                if key in self.bonus_channels:
                    # print ("index : ",self.layer_names)
                    layer_idx = self.layer_names.index(key+str(0))
                else:
    ...
    def update_layers_pointcloud(self, points_all, channels, R, t, elevation_map):
        ...
        """
        # add channels provided by user throught channels_should_add_to_pcl_for_exec_fusion
        new_channels=channels+list(self.bonus_channels)
        # print ("all_new channels: ",new_channels)
        process_channels, additional_fusion = self.get_fusion(
            new_channels, self.param.pointcloud_channel_fusions, self.layer_specs_points
        )
        # print(" all process channels: ", process_channels)
        # If channels has a new layer that is not in the semantic map, add it
        for channel in process_channels:
            if channel not in self.layer_names or channel+str(0) not in self.layer_names:
                # print(f"Layer {channel} not found, adding it to the semantic map")
                # add number of layers wanted by user
                if channel in self.bonus_channels:
                    for i in range(int(self.bonus_channels[channel])):
                       self.add_layer(channel+str(i))
                else:
