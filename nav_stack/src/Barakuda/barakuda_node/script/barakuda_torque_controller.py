...
class Controller():
    ...
    def __init__(self,name):
        ...
        # Parameters
        self.rate = rospy.get_param("barakuda_controller_frequency", 8.) #frequency in Hz
        ...
        self.vel_rot_pid = PID(Kp = 0.75, Ki = 0.15, Kd = 0.02, dt = 1/20)
        ...
        rospy.Subscriber("/odometry/filtered/local", Odometry, self.odom_cb)
    ...
    def cmd_vel_cb(self, cmd_vel):
        ...
        if(not self.manager_stop.data):
            ...
            if((self.odom != Odometry()) & self.use_cmd_vel_pid.data): # Check is the Odometry is not null   TO DO: ADD Other safety such as timeout
                ...
                self.cmd_vel_correct.angular.z = self.vel_rot_pid.update(cmd_vel.angular.z, self.odom.twist.twist.angular.z)
            else:
