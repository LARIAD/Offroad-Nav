    ...
    def publishCameraState(self):
        ...
        try:
            if self.axis.last_camera_position is not None:
                ...
                # Send tilt and pan TF 
                self.br.sendTransform((0,0, 0.065),
                     tf.transformations.quaternion_from_euler(0, 0, -msg.pan),
                     rospy.Time.now(),
                     "axis_pan",
                     "axis_base")

                self.br.sendTransform((0, 0, 0.155),
                     tf.transformations.quaternion_from_euler(0, -msg.tilt,0),
                     rospy.Time.now(),
                     "axis_tilt",
                     "axis_pan")

                self.br.sendTransform((0, -0.11, 0),
                     tf.transformations.quaternion_from_euler(-pi/2, 0, -pi/2),
                     rospy.Time.now(),
                     "axis_camera",
                     "axis_tilt")
...
class AxisPTZ:
    ...
    def __init__(self, axis_camera, args):
        ...
        self.st = None
        self.pub = rospy.Publisher("state/position", Ptz, queue_size=1)
    ...
    # Init on subscribe... replaced by init on the constructor
