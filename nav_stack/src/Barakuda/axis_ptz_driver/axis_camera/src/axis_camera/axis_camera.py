...
class StreamThread(threading.Thread):
    ...
    def run(self):
        ...
        while(True):
            if self.axis.use_gst:
                #rospy.logwarn("type= "+str(type(self.axis.width))) 
                #pipegst=f"rtspsrc location=rtsp://root:admin@192.168.0.90:554/axis-media/media.amp latency=0 ! decodebin max-size-time=30000000000 ! nvvidconv ! videoconvert ! videoscale ! video/xraw,width={str(self.axis.width)},height={str(self.axis.height)} ! autovideosink"
                #self.cap=cv2.VideoCapture(pipegst)
                #self.getimage_gst()
                pass
            else:
                self.stream()
    ...
    def getimage_gst(self):
        '''capture video with opencv gstreamer stream'''
        rospy.logwarn("is open "+str(self.cap.isOpened()))
