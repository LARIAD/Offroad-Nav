...
void VirtualLayer::onInitialize()
{
    ...
    _should_transform = _map_frame != layered_costmap_->getGlobalFrameID();
    if (_should_transform) {
        ROS_INFO("virtual_costmap_layer objects will be reprojected in the correct frame.");
    }
    ...
    nh.param<double>("tf_acceptable_delay", _tf_acceptable_delay, 1.0);
    nh.param<bool>("tf_strict", _tf_strict, false);

    // compute map bounds for the current set of areas and obstacles.
    computeMapBounds(true);
...
void VirtualLayer::reconfigureCb(VirtualLayerConfig &config, uint32_t level)
{
    ...
    _tf_acceptable_delay = config.tf_acceptable_delay;
    _tf_strict = config.tf_strict;
}
                                ...
                                double *min_x, double *min_y, double *max_x, double *max_y)
{
    ...
    if (!_polygon_changed && _obstacle_points.empty() && _zone_polygons.empty() && _obstacle_polygons.empty()) {
        return;
    }
...
void VirtualLayer::updateCosts(costmap_2d::Costmap2D &master_grid, int min_i, int min_j, int max_i, int max_j)
{
    ...
    //    return;
    //}
    //ROS_WARN("ok ça passe");

    if (_should_transform) {
        try {
            _map_frame_to_costmap_frame = tf_->lookupTransform(
                 layered_costmap_->getGlobalFrameID(),
                 _map_frame,
                 ros::Time::now(),
                 ros::Duration(.1));
        } catch (tf2::ExtrapolationException e) {
            ROS_WARN("Exception while get tf to reproject virtual zones' points:%s", e.what());
        }

        if (_map_frame_to_costmap_frame.header.stamp - ros::Time::now() > ros::Duration(_tf_acceptable_delay)) {
            if (_tf_strict) {
                throw tf2::ExtrapolationException("The saved tf is too hold (" + std::to_string(_tf_acceptable_delay) + " s) for virtual_costmap_layer to continue.");
            } else {
                ROS_ERROR("The saved tf is too hold (>%f s) for virtual_costmap_layer to properly function!", _tf_acceptable_delay);
            }
        }
    }
    ...
    for (int i = 0; i < _zone_polygons.size(); ++i) {
        setPolygonCost(master_grid, _zone_polygons[i], costmap_2d::LETHAL_OBSTACLE, min_i, min_j, max_i, max_j, false, _map_frame_to_costmap_frame);
    }

    // set costs of obstacle polygons
    for (int i = 0; i < _obstacle_polygons.size(); ++i) {
        setPolygonCost(master_grid, _obstacle_polygons[i], costmap_2d::LETHAL_OBSTACLE, min_i, min_j, max_i, max_j, true, _map_frame_to_costmap_frame);
    }
    ...
    if (_should_transform) {
        for (int i = 0; i < _obstacle_points.size(); ++i) {
            ...
            in.x = _obstacle_points[i].x;
            in.y = _obstacle_points[i].y;
            tf2::doTransform(in, out, _map_frame_to_costmap_frame);
            unsigned int mx, my;
            if (master_grid.worldToMap(out.x, out.y, mx, my)) {
    ...
    if (_polygon_changed) {
        // the polygons were changed, so at this iteration the bounds were the one containing the new and old polygons to ensure
        // it would erase any polygon that were removed
        // now that they were (potentially) removed, the bounds can be reduced to only fit the current polygon.
        computeMapBounds(true);
        _polygon_changed = false;
    }
}

void VirtualLayer::computeMapBounds(bool resetBounds)
{
    // reset bounds
    if (resetBounds) {
        _min_x = _min_y = _max_x = _max_y = 0;
    ...
    // do one if to avoid branching in for-loops
    if (_should_transform) {
        // iterate zone polygons
        for (int i = 0; i < _zone_polygons.size(); ++i) {
            for (int j = 0; j < _zone_polygons.at(i).size(); ++j) {
                geometry_msgs::Point in, out;
                in.x = _zone_polygons.at(i).at(j).x;
                in.y = _zone_polygons.at(i).at(j).y;
                tf2::doTransform(in, out, _map_frame_to_costmap_frame);
                double px = out.x;
                double py = out.y;
        ...
        for (int i = 0; i < _obstacle_polygons.size(); ++i) {
            for (int j = 0; j < _obstacle_polygons.at(i).size(); ++j) {
                geometry_msgs::Point in, out;
                in.x = _obstacle_polygons.at(i).at(j).x;
                in.y = _obstacle_polygons.at(i).at(j).y;
                tf2::doTransform(in, out, _map_frame_to_costmap_frame);
                double px = out.x;
                double py = out.y;
                _min_x = std::min(px, _min_x);
        ...
        for (int i = 0; i < _obstacle_points.size(); ++i) {
            geometry_msgs::Point in, out;
            in.x = _obstacle_points.at(i).x;
            in.y = _obstacle_points.at(i).y;
            tf2::doTransform(in, out, _map_frame_to_costmap_frame);
            double px = out.x;
            double py = out.y;
            _min_x = std::min(px, _min_x);
...
void VirtualLayer::setPolygonCost(costmap_2d::Costmap2D &master_grid, const Polygon &polygon, unsigned char cost,
                                  ...
                                  const geometry_msgs::TransformStamped &map_frame_to_costmap_frame)
{
    std::vector<PointInt> map_polygon;
    // when the costmap is not in the _map_frame, the points of the zones need to be transformed
    // do one if to avoid branching in for-loops
    if (_should_transform) {
        for (unsigned int i = 0; i < polygon.size(); ++i) {
            ...
            in.x = polygon[i].x;
            in.y = polygon[i].y;
            tf2::doTransform(in, out, map_frame_to_costmap_frame);
            master_grid.worldToMapNoBounds(out.x, out.y, loc.x, loc.y);
...
{
    // TODO maybe reuse Costmap2D::raytraceLine or Costmap2D::bresenham2D
    int dx = abs(x1 - x0);
    int dy = abs(y1 - y0);
    PointInt pt;
    pt.x = x0;
...
void VirtualLayer::zoneCallback(const barakuda_manager::Polygon2ConstPtr &zone_msg)
{
    if (zone_msg->zone.points.size() > 2) {
        ...
        for (int i = 0; i < zone_msg->zone.points.size(); ++i) {
            vector_to_add.push_back(toPoint(zone_msg->zone.points[i]));
        }
        ...
        // do reset bounds as it will be needed to reset part of the costmap that (can) become empty as the zones changed
        _polygon_changed = true;
        computeMapBounds(false);
    } else {
        ROS_ERROR_STREAM(tag << "A zone Layer needs to be a polygon with minimun 3 edges");
    }
}

void VirtualLayer::obstaclesCallback(const barakuda_manager::Polygon2ArrayConstPtr &obstacles_msg)
{
    std::lock_guard<std::mutex> l(_data_mutex);
    ...
    for (int i = 0; i < obstacles_msg->zones.size(); ++i) {
        Polygon vector_to_add;
        if (obstacles_msg->zones[i].zone.points.size() == 1) {
            if (obstacles_msg->zones[i].zone.points[0].z == 0.0) {
                ROS_INFO_STREAM(tag << "Adding a Point");
                _obstacle_points.push_back(toPoint(obstacles_msg->zones[i].zone.points[0]));
            } else if (obstacles_msg->zones[i].zone.points[0].z > 0.0) {
                ...
                for (int j = 0; j < N; ++j) {
                    ...
                    pt.x = obstacles_msg->zones[i].zone.points[0].x + cos(angle) * obstacles_msg->zones[i].zone.points[0].z;
                    pt.y = obstacles_msg->zones[i].zone.points[0].y + sin(angle) * obstacles_msg->zones[i].zone.points[0].z;
                    vector_to_add.push_back(pt);
                }
                _obstacle_polygons.push_back(vector_to_add);
            }
        } else if (obstacles_msg->zones[i].zone.points.size() == 2) {
            ROS_INFO_STREAM(tag << "Adding a Line");

            geometry_msgs::Point point_A = toPoint(obstacles_msg->zones[i].zone.points[0]);
            geometry_msgs::Point point_B = toPoint(obstacles_msg->zones[i].zone.points[1]);
            ...
            for (int j = 0; j < obstacles_msg->zones[i].zone.points.size(); ++j) {
                // fix Point 32 to point 64
                geometry_msgs::Point point64;
                point64.x = obstacles_msg->zones[i].zone.points[j].x;
                point64.y = obstacles_msg->zones[i].zone.points[j].y;
                point64.z = obstacles_msg->zones[i].zone.points[j].z;
                vector_to_add.push_back(point64);
            }
            _obstacle_polygons.push_back(vector_to_add);
        }
    }

    // do not reset bounds as it will be needed to reset part of the costmap that (can) become empty as the zones changed
    _polygon_changed = true;
    computeMapBounds(false);
}
