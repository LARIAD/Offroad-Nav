...
private:
    ...
    void computeMapBounds(bool resetBounds);
    ...
    void zoneCallback(const barakuda_manager::Polygon2ConstPtr &msg);

    /// \brief obstacle callback function
    void obstaclesCallback(const barakuda_manager::Polygon2ArrayConstPtr &obstacles_msg);
    ...
    bool _polygon_changed = false; // if the map should reset as the polygons changed
    double _min_x, _min_y, _max_x, _max_y; // cached map bounds

    bool _should_transform;
    double _tf_acceptable_delay;
    bool _tf_strict;

    std::vector<ros::Subscriber> _subs; // vector to save all ros subscribers

    geometry_msgs::TransformStamped _map_frame_to_costmap_frame;
};
